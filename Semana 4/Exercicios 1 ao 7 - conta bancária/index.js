...
const adicionarOpcao = (value, text) => {
...
const populaSelect = () => {
  ...
  adicionarOpcao('0', '(Selecione)')

  contasClientes.forEach(conta => {
    adicionarOpcao(conta.id, conta.nome)
  })
}
...
const sacar = (contaAtual, valor) => {
  ...
  if (!validarSaldo(valor, contaAtual.saldo)) {
    exibirMensagem(
      `Saldo insuficiente, (lembre-se: você é pobre!) Saldo atual: ${contaAtual.saldo}`
    )
    return
  }
